CREATE PROCEDURE [OWAPv2].[uspUpdateCustomerEmailAddress]
@SessionID [dbo].[SessionID]=null, @AuditID [dbo].[AuditID]=null, @UserPartyRoleID [dbo].[PartyRoleID]=null, @PartyID [dbo].[PartyID], @EmailAddress [dbo].[EmailAddress]=null, @RowCount INT OUTPUT, @ErrorCode INT OUTPUT

AS

/*
	Purpose:	OWAP Email Address Update

	Version		Date			Developer			Comment
	1.1			2017-03-03		Chris Ledger		BUG 13653 - Add Japan Update to CaseContactMechanismID and CaseDetails
	1.2			2017-03-10		Chris Ledger		BUG 13653 - Japan Update Add To BestPartyEmailAddress if Doesn't Exist
	1.3			2017-03-17		Chris Ledger		BUG 13653 - Fix Bug IN Adding of ContactMechanismID to BestPartyEmailAddress 
*/
...
BEGIN TRY
	...
	SELECT 	@chkSum = CHECKSUM(@EmailAddress)

	--CHECKSUM IS VALID
	IF @chkSum <> 0 
				
		--CHECK IF EMAIL ADDRESS IS ALREADY ASSOCIATED TO THE PARTY
		IF NOT EXISTS ( SELECT *
						FROM		[Meta].[PartyBestEmailAddresses] PBEA 
						INNER JOIN	[ContactMechanism].[PartyContactMechanisms] PCM ON PBEA.[ContactMechanismID] = PCM.[ContactMechanismID] AND PBEA.PartyID = PCM.PARTYID
						INNER JOIN	[ContactMechanism].[EmailAddresses] EA ON PCM.[ContactMechanismID] = EA.[ContactMechanismID]  AND EA.[EmailAddressChecksum] = @chkSum 
		...
		BEGIN TRAN
				...
				WHERE		A.AuditItemID IS NULL
							AND LTRIM(RTRIM(ISNULL(CUEA.EmailAddress,''))) <> '' 
				...
				--CHECK IF NEW EMAIL IS BLACKLISTED, IF IT ISN'T WE CAN'T USE IT 
				IF NOT EXISTS (
							SELECT	* 
							FROM	ContactMechanism.BlacklistContactMechanisms 
							WHERE	ContactMechanismID = (SELECT ContactMechanismID FROM @UD_EmailAddress)
						  )
						UPDATE	[Meta].[PartyBestEmailAddresses]
						SET		ContactMechanismID = (SELECT ContactMechanismID FROM @UD_EmailAddress)
						WHERE	[PartyID] = @PartyID


				------------------------------------------------------------------------------------------
				-- V1.1 UPDATE JAPAN CASE CONTACT MECHANISM AND CASE DETAILS WITH LATEST EMAIL ADDRESS
				------------------------------------------------------------------------------------------

				-----------------------------------------------------------------------------------------
				-- V1.2 INSERT NEW EMAIL ADDRESS INTO BestPartyEmail ADDRESS TABLE IF DOESN'T EXIST
				-----------------------------------------------------------------------------------------
				;WITH	NewEmailAddress ( ContactMechanismID, PartyID, CaseID,  AuditItemID, BestPartyEmailAddressID ) AS
				(
					SELECT CUEA.ContactMechanismID AS ContactMechanismID ,	-- V1.3
							CD.PartyID ,
							MAX(CD.CaseID) AS CaseID,
							MAX(APCM.AuditItemID) AS AuditItemID,
							BEA.ContactMechanismID AS BestPartyEmailAddressID
					FROM     Meta.CaseDetails CD
					INNER JOIN @UD_EmailAddress CUEA ON CD.PartyID = CUEA.PartyID	-- V1.3
					LEFT JOIN [Sample_Audit].Audit.PartyContactMechanisms APCM ON APCM.PartyID = CD.PartyID AND APCM.ContactMechanismID = CUEA.ContactMechanismID
					LEFT JOIN Meta.PartyBestEmailAddresses BEA ON CD.PartyID = BEA.PartyID
					WHERE CD.PartyID = @PartyID
					AND CD.Country = 'Japan'
					AND BEA.ContactMechanismID IS NULL
					GROUP BY CUEA.ContactMechanismID, BEA.ContactMechanismID, CD.PartyID	-- V1.3
				 )
				INSERT INTO Meta.PartyBestEmailAddresses (PartyID, ContactMechanismID, AuditItemID)
				SELECT	NA.PartyID, NA.ContactMechanismID, NA.AuditItemID 
				FROM NewEmailAddress NA
				-----------------------------------------------------------------------------------------

				-----------------------------------------------------------------------------------------
				-- V1.1 UPDATE Sample.Meta.CaseDetails TO REFERENCE UPDATED EMAIL ADDRESS
				-----------------------------------------------------------------------------------------
				;WITH	NewEmailAddress ( ContactMechanismID, PartyID, CaseID ) AS
				(
					SELECT BEA.ContactMechanismID AS ContactMechanismID ,
							CD.PartyID ,
							MAX(CD.CaseID) AS CaseID
					FROM     Meta.CaseDetails CD
					...
					WHERE CD.PartyID = @PartyID
					AND CD.Country = 'Japan'
					AND ISNULL(CD.EmailAddressContactMechanismID,'') <> ISNULL(BEA.ContactMechanismID,'')
					GROUP BY BEA.ContactMechanismID, CD.PartyID
				 )
				UPDATE  CD
				SET     CD.EmailAddressContactMechanismID = NA.ContactMechanismID
				--SELECT *
				FROM    Meta.CaseDetails CD
				INNER JOIN NewEmailAddress NA ON CD.CaseID = NA.CaseID
				-----------------------------------------------------------------------------------------

				-----------------------------------------------------------------------------------------
				-- V1.1 DELETE EMAIL CaseContactMechanisms
				-----------------------------------------------------------------------------------------
				;WITH	NewEmailAddress ( ContactMechanismID, PartyID, CaseID ) AS
				(
					SELECT BEA.ContactMechanismID AS ContactMechanismID ,
					...
					LEFT JOIN ContactMechanism.EmailAddresses EA ON CD.EmailAddressContactMechanismID = EA.ContactMechanismID
					LEFT JOIN Meta.PartyBestEmailAddresses BEA ON CD.PartyID = BEA.PartyID
					LEFT JOIN ContactMechanism.EmailAddresses EA1 ON BEA.ContactMechanismID = EA1.ContactMechanismID
					WHERE CD.PartyID = @PartyID
					AND CD.Country = 'Japan'
					AND ISNULL(CD.EmailAddressContactMechanismID,'') <> ISNULL(BEA.ContactMechanismID,'')
					AND EA1.ContactMechanismID > 0
					GROUP BY BEA.ContactMechanismID, CD.PartyID
				 )
				--SELECT *
				DELETE FROM Event.CaseContactMechanisms
				FROM    Event.CaseContactMechanisms CCM
				INNER JOIN NewEmailAddress NA ON CCM.CaseID = NA.CaseID
				WHERE CCM.ContactMechanismTypeID = 6
				-----------------------------------------------------------------------------------------


				-----------------------------------------------------------------------------------------
				--  V1.1 INSERT CaseContactMechanisms TO REFERENCE UPDATED EMAIL ADDRESS
				-----------------------------------------------------------------------------------------
				;WITH	NewEmailAddress ( ContactMechanismID, PartyID, CaseID ) AS
				(
					SELECT BEA.ContactMechanismID AS ContactMechanismID ,
							...
							MAX(CD.CaseID) AS CaseID
					FROM     Meta.CaseDetails CD
					LEFT JOIN Event.CaseContactMechanisms CCM ON CD.CaseID = CCM.CaseID AND CCM.ContactMechanismTypeID = 6
					LEFT JOIN ContactMechanism.EmailAddresses EA ON CD.EmailAddressContactMechanismID = EA.ContactMechanismID
					LEFT JOIN Meta.PartyBestEmailAddresses BEA ON CD.PartyID = BEA.PartyID
					LEFT JOIN ContactMechanism.EmailAddresses EA1 ON BEA.ContactMechanismID = EA1.ContactMechanismID
					WHERE CD.PartyID = @PartyID
					AND CD.Country = 'Japan'
					AND EA1.ContactMechanismID > 0
					AND CCM.CaseID IS NULL
					GROUP BY BEA.ContactMechanismID, CD.PartyID
				 )
				INSERT INTO Event.CaseContactMechanisms (CaseID, ContactMechanismTypeID, ContactMechanismID)
				SELECT CD.CaseID, 6, NA.ContactMechanismID
				--SELECT *
				FROM     Meta.CaseDetails CD
				INNER JOIN NewEmailAddress NA ON CD.CaseID = NA.CaseID
				LEFT JOIN Event.CaseContactMechanisms CCM ON CD.CaseID = CCM.CaseID AND CCM.ContactMechanismTypeID = 6
