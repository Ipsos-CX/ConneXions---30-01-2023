CREATE PROCEDURE [OWAPv2].[uspGetCustomerDetailsFromNamesAndPostcode]
@FirstName [dbo].[NameDetail]=N'', @Initials [dbo].[NameDetail]=N'', @LastName [dbo].[NameDetail]=N'', @OrganisationName [dbo].[OrganisationName]=N'', @PostCode [dbo].[Postcode]=N'', @RowCount INT OUTPUT, @ErrorCode INT OUTPUT
AS
SET NOCOUNT ON
...
/*
	...
	1.0				?				?					Created
	1.1				10/05/2018		Chris Ross			BUG 14399 - Add in filter to ensure Person is not GDPR erased
...
BEGIN TRY
	...
	BEGIN
		...
		AND (PP.PartyID IS NULL																-- v1.1
			 OR NOT EXISTS (SELECT er.PartyID FROM [$(AuditDB)].GDPR.ErasureRequests er
							WHERE er.PartyID = PP.PartyID)
			)
