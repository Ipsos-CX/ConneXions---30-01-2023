CREATE PROC SelectionOutput.uspAuditChinaServiceWithResponsesOutput
(
...
/*
	Purpose:	Audit the China With Responses output 
	
	Version			Date			Developer			Comment
	1.0				27-07-2015		Chris Ross			Created
	1.1				31-03-2016		Chris Ross			BUG 12407: Add ITYPE and PilotCode into Audit.SelectionOutput.
*/
...
BEGIN TRY
	...
	BEGIN TRAN
		...
		INNER JOIN Sample_ETL.China.Service_WithResponses CSE ON O.ID = CSE.CaseID 	
		...
		BEGIN
			...
			SELECT DISTINCT
				O.[AuditID], 
				O.[AuditItemID], 
				(SELECT SelectionOutputTypeID FROM [$(AuditDB)].dbo.SelectionOutputTypes WHERE SelectionOutputType = 'Email') AS [SelectionOutputTypeID],
				S.[PartyID], 
				S.[ID] AS [CaseID], 
				S.[FullModel], 
				S.[Model], 
