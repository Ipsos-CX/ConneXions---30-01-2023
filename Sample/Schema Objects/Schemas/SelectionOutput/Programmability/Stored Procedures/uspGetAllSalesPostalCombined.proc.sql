CREATE PROCEDURE [SelectionOutput].[uspGetAllSalesPostalCombined]
		
		@Brand			[dbo].[OrganisationName], 
		@Questionnaire	[dbo].[Requirement],
		@Market			[dbo].[Country],
		@Lang			[dbo].[LanguageID]
...
/*
	Purpose: Postal Combined Output
		
	Version			Date			Developer			Comment
	1.0				$(ReleaseDate)	Eddie Thomas		Created
	1.1				2018-02-06		Chris Ledger		BUG 14526: Order by ID as well as lang so bilingual files output with same order
...
SELECT *
FROM	Postal_CTE pc
ORDER BY row_number() OVER (PARTITION BY lang ORDER BY ID), lang		-- V1.1