CREATE PROCEDURE [SelectionOutput].[uspAuditSMSOutput]
@FileName VARCHAR (255)
AS

/*
	Purpose:	Audit the SMS output
	
	Version			Date			Developer			Comment
	1.1				23-10-2017		Chris Ledger		Add BilingualFlag, DearNameBilingual, langBilingual
	1.2				10-10-2019		Ben King			BUG 15581 change URL
*/
...
BEGIN TRY
	...
	BEGIN TRAN
		...
		SELECT DISTINCT
			ID, 
			PartyID,
			(SELECT CaseOutputTypeID FROM Event.CaseOutputTypes WHERE CaseOutputType = 'SMS') AS CaseOutputTypeID
		...
		BEGIN
			...
			SELECT DISTINCT
				O.[AuditID], 
				O.[AuditItemID], 
				(SELECT SelectionOutputTypeID FROM [Sample_Audit].dbo.SelectionOutputTypes WHERE SelectionOutputType = 'SMS') AS [SelectionOutputTypeID],
				S.[PartyID], 
				S.[ID] AS [CaseID], 
				S.[FullModel], 
				S.[Model], 
				...
				S.[WorkPhone],
				S.[MobilePhone],
				'https://feedback.tell-jlr.com/S/' +  S.Password, --v1.2
				S.BilingualFlag,			-- V1.1
				S.DearNameBilingual,		-- V1.1
				S.langBilingual				-- V1.1				
				
			FROM #OutputtedSelections O
			INNER JOIN SelectionOutput.SMS S ON O.CaseID = S.[ID]
