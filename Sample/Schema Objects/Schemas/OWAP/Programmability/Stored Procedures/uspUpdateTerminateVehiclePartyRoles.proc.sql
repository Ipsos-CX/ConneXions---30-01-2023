CREATE    PROCEDURE [OWAP].[uspUpdateTerminateVehiclePartyRoles]
(
		...
		BEGIN
			...
			(
				SELECT 
					vpr.VehicleID 
				FROM 
					[Sample].[Vehicle].vwDA_VehiclePartyRoles AS vpr 
				WHERE 
				...
				BEGIN
					...
					INSERT INTO
						[Sample].[Vehicle].vwDA_VehiclePartyRoles
						(
							PartyID, 
							VehicleRoleTypeID, 
							VehicleID, 
...
----Terminate all roles
	...
	JOIN	Sample.Vehicle.Vehicles V on V.VehicleID = @VehicleID
	WHERE	cd.VIN = V.VIN
