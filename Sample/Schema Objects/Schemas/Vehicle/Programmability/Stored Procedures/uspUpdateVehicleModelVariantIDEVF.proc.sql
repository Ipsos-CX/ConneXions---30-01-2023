CREATE PROCEDURE [Vehicle].[uspUpdateVehicleModelVariantIDEVF]
	...
	*/
	...
	/* SET LOCAL CONNECTION SETTINGS */
		...
		BEGIN TRY
			...
			UPDATE V
			SET V.ModelVariantID = 
				CASE 
					WHEN V.ModelID = 2 AND V.ModelYear BETWEEN 1999 AND 2008 THEN 4			--'S-TYPE (x200 1999-2008)'
					WHEN V.ModelID = 2 AND V.ModelYear IS NULL THEN 3						--'S-TYPE (Generic)'	
					WHEN V.ModelID = 3 AND V.ModelYear <= 2015 THEN 6						--'XF (x250 2008-)'
					WHEN V.ModelID = 3 AND V.ModelYear >= 2016 THEN 44						--'XF (x250 2016-)'
					WHEN V.ModelID = 3 AND V.ModelYear IS NULL THEN 5						--'XF (Generic)'
					WHEN V.ModelID = 4 AND V.ModelYear BETWEEN 2003 AND 2010 THEN 10		--'XJ (x350 2003-2010)'
					WHEN V.ModelID = 4 AND V.ModelYear > 2010 THEN 11						--'XJ (x351 2010-)'
					WHEN V.ModelID = 4 AND V.ModelYear IS NULL THEN 9						--'XJ (Generic)'	
					WHEN V.ModelID = 5 AND V.ModelYear BETWEEN 1997 AND 2006 THEN 13		--'XK (x100 1997-2006)'
					WHEN V.ModelID = 5 AND V.ModelYear > 2006 THEN 14						--'XK (x150 2006-)'
					WHEN V.ModelID = 5 AND V.ModelYear IS NULL THEN 12						--'XK (Generic)'	
					WHEN V.ModelID = 6 AND V.ModelYear BETWEEN 2001 AND 2009 THEN 16		--'X-TYPE (x400 2001-2009)'
					WHEN V.ModelID = 6 AND V.ModelYear IS NULL THEN 15						--'X-TYPE (Generic)'	
					WHEN V.ModelID = 8 THEN 17												--'Defender'
					WHEN V.ModelID = 9 AND V.ModelYear <= 2004 THEN 18						--'Discovery (<2004)'
					WHEN V.ModelID = 9 AND V.ModelYear IS NULL THEN 19						--'Discovery (Generic)'
					WHEN V.ModelID = 10 AND V.ModelYear BETWEEN 2004 AND 2008 THEN 21		--'Discovery 3 (L319 2004-2009)'
					--WHEN V.ModelID = 10 AND V.ModelYear >= 2009 THEN 22					--'Discovery 4 (L319 2009)'					-- V1.23
					WHEN V.ModelID = 11 AND V.ModelYear BETWEEN 1996 AND 2005 THEN 24		--'Freelander (L314 1996-2006)'
					WHEN V.ModelID = 11 AND V.ModelYear IS NULL THEN 23						--'Freelander (Generic)'	
					WHEN V.ModelID = 12 AND V.ModelYear >= 2006 THEN 26						--'Freelander 2 (L359 2006-)'
					WHEN V.ModelID = 12 AND V.ModelYear IS NULL THEN 25						--'Freelander 2 (Generic)'	
					WHEN V.ModelID = 13 AND V.ModelYear < 2001 THEN 28						--'Range Rover (<2001)'
					WHEN V.ModelID = 13 AND V.ModelYear >= 2001 THEN 30						--'Range Rover (L322 2001-)'
					WHEN V.ModelID = 13 AND V.ModelYear IS NULL THEN 29						--'Range Rover (Generic)'	
					...
					WHEN V.ModelID = 14 AND V.ModelYear >= 2011 THEN 32						--'Range Rover Evoque (L538 2011-)'
					WHEN V.ModelID = 14 AND V.ModelYear IS NULL THEN 31						--'Range Rover Evoque (Generic)'	
					WHEN V.ModelID = 15 AND V.ModelYear >= 2005 AND V.VIN NOT LIKE 'SALWA%' THEN 34 --'Range Rover Sport (L320 2005-)'   -- V1.2
					WHEN V.ModelID = 15 AND V.ModelYear >= 2014 AND V.VIN LIKE 'SALWA%' THEN 37		--'Range Rover Sport (L494)'		 -- V1.2
					WHEN V.ModelID = 15 AND V.ModelYear IS NULL THEN 33						--'Range Rover Sport (Generic)'	
					WHEN V.ModelID = 16 THEN 22												--'Discovery 4 (L319 2009)'						
					WHEN V.ModelID = 21 AND V.ModelYear < 2016 THEN 7						--'XF Sportbrake'
					WHEN V.ModelID = 21 AND V.ModelYear >= 2016 THEN 45						--'XF Sportbrake (x250 2016-)'
					WHEN V.ModelID = 21 AND V.ModelYear IS NULL THEN 46						--'XF Sportbrake (Generic)'
					...
					WHEN V.ModelID = 23 AND V.VIN LIKE 'SAJ__6[0-7]____[KP]%' AND V.ModelYear >= 2014 THEN 36	--'F-TYPE (x152 2014-)'	-- V1.9
					...
					WHEN V.ModelID = 28 AND V.ModelYear >= 2016 THEN 48						--'F-PACE (X761 2016-)'
					WHEN V.ModelID = 28 AND V.ModelYear IS NULL THEN 47						--'F-PACE (Generic)'
					...
					ELSE NULL
				END
			FROM Vehicle.ExtraVehicleFeed V
