CREATE PROCEDURE [dbo].[uspGET_ServiceSampleAnalysisData_12MonthRolling_7Years_5_Events]
@RunDate DATE
AS
SET XACT_ABORT ON
		--SET NOCOUNT ON
...
--	SELECT * FROM #AfterSaleDataset
	...
	/* MR: COUNT THE NUMBER OF DISTINCT EVENTS PER VEHICLE IN THE LAST 3 TO 6 YEARS */
		...
		--FLAG THE MARKET TO APPEAR IN THE REPORT --v3.3
		UPDATE ASD
	...
	/* ADD COUNT OF VEHICLES WITH +2 SERVICE EVENTS WITHIN 36MIS */	
			...
			INNER JOIN
				(
					...
					FROM #AfterSaleDataset
					WHERE ServiceEventCount36MIS >= 5 AND
									CountryAggregate = 1
					GROUP BY 
						Manufacturer
						, Market
	...
	/* ADD COUNT OF VEHICLES WITH +2 SERVICE EVENTS BETWEEN 37MIS AND 72MIS */	
			...
			INNER JOIN
				(
					...
					FROM #AfterSaleDataset
					WHERE [ServiceEventCount37-72MIS] >= 5 AND
									CountryAggregate = 1
					GROUP BY 
						Manufacturer
						, Market
	...
	*/
	...
	INSERT INTO #JLREventsReport			----------COUNTRIES----------
		(Manufacturer, Market, YearOfSales, NumberOfSales, [+2Events36MIS], [+2Events36MIS %], [+2Events37-72MIS], [+2Events37-72MIS %], ManufacturerII, MarketII
			, YearOfSalesII, NumberOfSalesII, [+2Events36MISII], [+2Events36MIS %II], [+2Events37-72MISII], [+2Events37-72MIS %II], MarketOrder, CountryAggregate)
		...
		(Manufacturer, Market, YearOfSales, NumberOfSales, [+2Events36MIS], [+2Events36MIS %], [+2Events37-72MIS], [+2Events37-72MIS %], ManufacturerII, MarketII
			, YearOfSalesII, NumberOfSalesII, [+2Events36MISII], [+2Events36MIS %II], [+2Events37-72MISII], [+2Events37-72MIS %II], MarketOrder, RegionAggregate)
...
--select * from #ServiceRetention
	...
	--------- COUNTRIES ---------
		UPDATE R
			...
			, [+2Events37-72MIS %] = CAST(CAST(ROUND((CAST(SR.[+2EVENTS37-72MIS] AS DECIMAL(10, 2)) / CAST(SR.NumberOfSales AS DECIMAL(10, 2))) * 100, 0) AS INT) AS VARCHAR(5)) + '%'
		FROM #ServiceRetention_Country SR
		...
		UPDATE R
			...
			, [+2Events37-72MIS %II] = CAST(CAST(ROUND((CAST(SR.[+2EVENTS37-72MIS] AS DECIMAL(10, 2)) / CAST(SR.NumberOfSales AS DECIMAL(10, 2))) * 100, 0) AS INT) AS VARCHAR(5)) + '%'
		FROM #ServiceRetention_Country SR
