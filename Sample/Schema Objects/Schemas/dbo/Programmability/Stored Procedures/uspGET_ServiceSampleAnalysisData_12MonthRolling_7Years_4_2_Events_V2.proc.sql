CREATE PROCEDURE [dbo].[uspGET_ServiceSampleAnalysisData_12MonthRolling_7Years_4_2_Events_V2] 
@RunDate DATE
AS
SET XACT_ABORT ON
		--SET NOCOUNT ON
	...
	/* 
		...
		3.5			20/01/2020			Chris Ledger		Bug 15372 - Fix database references and cases
	*/
...
--RETURN
		...
		--UPDATE t 
		--	SET DealerCode = x.OutletCode
		--	, Market = x.Market
		--	, Manufacturer = x.Manufacturer
		--FROM AfterSaleDataset_a T
		--JOIN (
		--	SELECT ROW_NUMBER() OVER (PARTITION BY SaleEventID ORDER BY DW.id DESC) AS rownos,
		--	SaleEventID,
		--	DW.OutletCode
		--	,DW.Market
		--	,DW.Manufacturer
		--FROM AfterSaleDataset_a t	
		--JOIN Event.EventPartyRoles epR ON t.SaleEventID = EPR.EventID
		--JOIN DW_JLRCSPDealers DW ON DW.OutletPartyID = EPR.PartyID
		--) AS X ON x.SaleEventID = t.SaleEventID
		--WHERE rownos = 1
	...
	/* ADD COUNT OF VEHICLES WITH +2 SERVICE EVENTS WITHIN 36MIS */	
			...
			INNER JOIN
				(
					...
					WHERE ServiceEventCount36MIS >= 4
					GROUP BY 
						Manufacturer
						, Market
						, YearOfSales
	...
	/* ADD COUNT OF VEHICLES WITH +2 SERVICE EVENTS BETWEEN 37MIS AND 72MIS */	
			...
			INNER JOIN
				(
					...
					WHERE [ServiceEventCount37-84MIS] >= 6
					GROUP BY 
						Manufacturer
						, Market
						, YearOfSales
	...
	*/
	...
	INSERT INTO #JLREventsReport
		(Manufacturer, Market, YearOfSales, NumberOfSales, [+2Events36MIS], [+2Events36MIS %], [+6Events37-84MIS], [+6Events37-84MIS %], ManufacturerII, MarketII
			, YearOfSalesII, NumberOfSalesII, [+2Events36MISII], [+2Events36MIS %II], [+6Events37-84MISII], [+6Events37-84MIS %II], MarketOrder)
...
*/
	...
	/* RETURN JLR SERVICE RETENTION 2 + SERVICE EVENTS WITHIN 36 MIS */
		...
		UPDATE R
			...
			, [+6Events37-84MIS %] = CAST(CAST(ROUND((CAST(SR.[+6Events37-84MIS] AS DECIMAL(7, 2)) / CAST(SR.NumberOfSales AS DECIMAL(7, 2))) * 100, 0) AS INT) AS VARCHAR(5)) + '%'
		FROM #ServiceRetention sr
		...
		UPDATE R
			...
			, [+6Events37-84MIS %II] = CAST(CAST(ROUND((CAST(SR.[+6Events37-84MIS] AS DECIMAL(7, 2)) / CAST(SR.NumberOfSales AS DECIMAL(7, 2))) * 100, 0) AS INT) AS VARCHAR(5)) + '%'
		FROM #ServiceRetention SR
