CREATE PROCEDURE [Selection].[uspRunSelection_NPSManual_ForExcelOutput]
@MIS VARCHAR (6), @ManufacturerPartyID TINYINT, @CountryID SMALLINT, @StartDays SMALLINT, @EndDays SMALLINT, @SelectionDate SMALLDATETIME
AS
SET NOCOUNT ON
    SET XACT_ABORT ON
...
-------------------------------------------------------------------------------
        ...
        INSERT  INTO Selection.NPS_SelectedEvents_ForExcelOutput
                ( SelectionDate ,
                  MIS ,
                  EventID ,
                  VehicleID ,
                ...
                SELECT DISTINCT
                 ...
                 --                                                  ) AS [ID] ,
                        ...
                        LTRIM(REPLACE(TNL.ContactNumber,'Mobile:','')) AS HomeNumber ,
                        LTRIM(REPLACE(TNP.ContactNumber,'Mobile:','')) AS WorkNumber ,
                        LTRIM(REPLACE(TNM.ContactNumber,'Mobile:','')) AS MobileNumber ,
                        ...
                        CASE WHEN ISNULL(EA.EmailAddress,'') <> '' THEN 'H' 
							 WHEN LTRIM(REPLACE(TNM.ContactNumber,'Mobile:','')) LIKE X.PrefixPattern THEN 'S'
							 WHEN LTRIM(REPLACE(TNL.ContactNumber,'Mobile:','')) LIKE Y.PrefixPattern THEN 'S'
                             WHEN LTRIM(REPLACE(TNP.ContactNumber,'Mobile:','')) LIKE Z.PrefixPattern THEN 'S'
			...
			--EPT
                        ...
                        LEFT JOIN sample.Selection.NPS_MobilePhoneCodes X ON (B.CountryID = X.CountryId) AND (LTRIM(REPLACE(TNM.ContactNumber,'Mobile:','')) LIKE X.PrefixPattern)
						LEFT JOIN sample.Selection.NPS_MobilePhoneCodes Y ON (B.CountryID = Y.CountryId) AND (LTRIM(REPLACE(TNL.ContactNumber,'Mobile:','')) LIKE Y.PrefixPattern)
						LEFT JOIN sample.Selection.NPS_MobilePhoneCodes Z ON (B.CountryID = Z.CountryId) AND (LTRIM(REPLACE(TNP.ContactNumber,'Mobile:','')) LIKE Z.PrefixPattern)                                                       
...
-----------------------------------------------------------------------------------------
	...
	PRINT '@MIS='+@Mis+', @ManufacturerPartyID='+ Convert(varchar(50),@ManufacturerPartyID)+' ,@CountryID='+Convert(varchar(50),@CountryID)
    END TRY
