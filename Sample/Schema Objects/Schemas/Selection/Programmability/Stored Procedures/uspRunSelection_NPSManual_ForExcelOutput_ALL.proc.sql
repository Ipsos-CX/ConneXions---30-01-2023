CREATE PROCEDURE [Selection].[uspRunSelection_NPSManual_ForExcelOutput_ALL]
	...
	[Selection].[uspRunSelection_NPSManual_ForExcelOutput_ALL]	'20150203','6 MIS',3,219
...
--------------------------------------------------------------------------------			
        ...
        SELECT  ISNULL(RespondentPartyID, '') AS PartyID ,							
                ...
                ISNULL(HomeNumber, '') AS HomeNumber ,
                ISNULL(WorkNumber, '') AS WorkNumber ,
                ...
                --CASE WHEN CLPSalesCaseID = 0 THEN '' ELSE ISNULL(CLPSalesCaseID, '') END AS CLPSalesCaseID ,
                --CASE WHEN CLPServiceCaseID = 0 THEN '' ELSE ISNULL(CLPServiceCaseID, '') END AS CLPServiceCaseID ,
                --CASE WHEN RoadsideCaseID = 0 THEN '' ELSE ISNULL(RoadSideCaseID, '') END AS RoadSideCaseID ,
                ISNULL(CLPSalesCaseID, '')  AS CLPSalesCaseID ,
                ISNULL(CLPServiceCaseID, '') AS CLPServiceCaseID ,
                ISNULL(RoadSideCaseID, '') AS RoadSideCaseID ,                
				ISNULL(NewUsed, '') AS NewUsed ,
                1 AS CarOwnership ,
                ISNULL(MIS, '') AS MIS ,
                @CountryName AS Market ,
                ISNULL(SuperNationalRegion, '') AS SuperNationalRegion
...
-----------------------------------------------------------------------------------------
 ...
 --       WHERE   CONVERT(date,SelectionDate) = @SelectionDate
 --               AND MIS = @MIS
 --               AND Manuf = @ManufacturerPartyID
 --               AND ccode = @CountryID 
 ...
 --       WHERE   (CONVERT(date,SelectionDate) = @SelectionDate) AND 
 --               (MIS = @MIS) AND
