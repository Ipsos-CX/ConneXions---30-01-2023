...
CREATE PROCEDURE [SampleReport].[uspCalcSampleVolumeBenchmarksDaily]
@ThreeMonth BIT = 0,
@SixMonth BIT = 0,
@Year BIT = 0
AS
...
/*
	Purpose:	Calculates Daily Benchmark figures by variable time period
...
BEGIN TRY
	...
	DECLARE @BuilIDMax INT = CASE
								 WHEN @ThreeMonth = 1 THEN 91
								 WHEN @SixMonth = 1 THEN 182
								 WHEN @Year = 1 THEN 365
							 END
	...
	AND BenchmarkType = 'Daily Calculation'
	...
	WHILE (@CountBack > -@BuilIDMax)
			...
			AS
				...
				WHERE SQ.LoadedDate >= DATEADD(DAY, -1, @StartDate)
				AND SQ.LoadedDate < @StartDate
				AND L.VolumeReportOutput = 1
				GROUP BY L.Brand, M.Market, L.Questionnaire
				)
							
			INSERT INTO [SampleReport].[SampleVolumeBenchmarks] (Market, Brand, Questionnaire, TimePeriod, StartDate, FileRow_LoadedCountDaily, ReportDate) 
			SELECT	L.Market,
					L.Brand,
					L.Questionnaire,
					(SELECT @Build) AS TimePeriod,
					DATEADD(DAY, -1, @StartDate) AS 'StartDate',
					FRC.FileRow_LoadedCount AS FileRow_LoadedCountDaily,
			...
			SET @StartDate =  DATEADD(DAY, -1, @StartDate) -- PREVIOUS DAY, MOVING BACKWARDS
	...
	;WITH CTE AS
		(   SELECT  Brand,
					Market,
					Questionnaire,
					FileRow_LoadedCountDaily, 
					[half1] = NTILE(2) OVER(PARTITION BY Brand,Market,Questionnaire ORDER BY FileRow_LoadedCountDaily), 
					[half2] = NTILE(2) OVER(PARTITION BY Brand,Market,Questionnaire ORDER BY FileRow_LoadedCountDaily DESC)
			FROM    [SampleReport].[SampleVolumeBenchmarks]
			WHERE   FileRow_LoadedCountDaily IS NOT NULL
		)
		...
		SELECT  Brand,
				...
				Questionnaire,
				(SELECT @Build) AS TimePeriod,
				'Daily Calculation' AS BenchmarkType,
				CAST(ROUND((MAX(CASE WHEN Half1 = 1 THEN FileRow_LoadedCountDaily END) + 
				MIN(CASE WHEN Half2 = 1 THEN FileRow_LoadedCountDaily END)) / 2.0, 0) AS NUMERIC) AS Median,
				CONVERT(date, GETDATE()) AS ReportDate
		FROM    CTE
