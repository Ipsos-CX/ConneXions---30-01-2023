CREATE PROCEDURE CustomerUpdate.uspPostalAddress_ExtractSubStreetNumber

AS

/*
	Purpose: Extract Street Number from SubStreetAndNumber field in CustomerUpdate.PostalAddress.

	Split House/Building number and Street in field 'SubStreetAndNumber' in CustomerUpdate.PostalAddress into their 
	own explicit fields.
	...
	SubStreetAndNumber		SubStreet Number			->		SubStreetNumber
	SubStreetAndNumber		SubStreet (incl. Prefix/Suffix)		->		SubStreet
	...
	1.0				$(ReleaseDate)		Simon Peacock		Created from [Prophet-ETL].dbo.uspIP_CUSTOMERUPDATE_ExtractSubStreetNumber
...
BEGIN TRY
	...
	SELECT AuditItemID, CountryID, SubStreetAndNumber
...
/* 	Standardise word delimiters. This MAY be country specific. Could store this data in a meta data table 
	...
	UPDATE PA
	SET PA.SubStreetNumber = H.Value
	...
	UPDATE PA
	SET PA.SubStreet = O.OutputString
	FROM  CustomerUpdate.PostalAddress PA
