CREATE PROCEDURE [Stage].[uspStandardise_LandRover_Spain_Sales_VISTA]
AS
...
BEGIN TRY
	...
	UPDATE	Stage.LandRover_Spain_Sales_VISTA
	...
	SET			PreferredLanguageId =	CASE 
										...
										END					
	FROM		Stage.LandRover_Spain_Sales_VISTA SSV	
	...
	IF Exists( 
				SELECT	ID 
				FROM	Stage.LandRover_Spain_Sales_VISTA 
				WHERE	ISNULL(CountryID,0)	= 0	OR 
						ISNULL(PreferredLanguageId,0) =0
			)			 
			
			RAISERROR(	N'CountryID / PreferredLanguageId cannot equal zero (Stage.LandRover_Spain_Sales_VISTA).', 
						16,
						1
					 )
...
BEGIN CATCH
	...
	EXEC('
		SELECT *
		INTO [$(ErrorDB)].Stage.LandRover_Spain_Sales_VISTA_' + @TimestampString + '
		FROM Stage.LandRover_Spain_Sales_VISTA
	')
