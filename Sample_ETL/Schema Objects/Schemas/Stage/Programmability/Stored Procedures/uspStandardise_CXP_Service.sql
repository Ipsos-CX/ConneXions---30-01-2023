CREATE PROCEDURE [Stage].[uspStandardise_CXP_Service]
/*
	BUGTRACKER ID: 18080 - CXP Service Data
	...
	LIVE			1.0				27/01/2021		Eddie Thomas		Created from [Sample_ETL].dbo.uspStandardise_CXP_Sales
...
BEGIN TRY
	...
	UPDATE	[Stage].CXP_Service
	SET		ConvertedServiceEventDate = CONVERT(DATETIME, [Service_Event_Date], 103)
	WHERE	ISDATE([Service_Event_Date]) = 1
	...
	IF Exists( 
			...
			RAISERROR(	N'Data in Stage.CXP_Service has missing Meta-Data.', 
						16,
						1
					 )
...
BEGIN CATCH
	...
	EXEC('
		SELECT *
		INTO [$(ErrorDB)].Stage.CXP_Service_' + @TimestampString + '
		FROM Stage.CXP_Service
	')
