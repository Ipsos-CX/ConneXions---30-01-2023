CREATE PROCEDURE [Stage].[uspStandardise_LandRover_Brazil_Sales_VISTA]
AS
...
/*
	...
	Original		22/01/2014		Chris Ross			Original Version
	1.0				16/04/2014		Martin Riverol		Copied and amended for LR Brazil
...
BEGIN TRY
	...
	UPDATE	Stage.LandRover_Brazil_Sales_VISTA
	...
	--UPDATE  Stage.Jaguar_France_Sales_VISTA
...
--	AND		isnull(CombinedDateOfBirth, '') = ''
	...
	--FROM	Stage.Jaguar_France_Sales_VISTA
...
BEGIN CATCH
	...
	EXEC('
		SELECT *
		INTO [$(ErrorDB)].Stage.LandRover_Brazil_Sales_VISTA_' + @TimestampString + '
		FROM Stage.LandRover_Brazil_Sales_VISTA
	')
