CREATE PROCEDURE [Stage].[uspStandardise_China_Global_Sales]
...
/*
	...
	1.10			22/03/2017		Chris Ledger		Bug 13657 - Map CustomerIdentifier to SurnameField1 or CompanyName
...
BEGIN TRY
	...
	FROM		[Stage].[Global_China_Sales_WithResponses]		GS
	...
	UPDATE	Stage.Global_China_Sales_WithResponses
	...
	IF Exists( 
				SELECT	ID 
				FROM	[Stage].[Global_China_Sales_WithResponses] 
			...
			RAISERROR(	N'Data in Stage.Global_China_Sales_WithResponses has missing Meta-Data.', 
						16,
						1
					 )
	...
	IF Exists( 		
			...
			RAISERROR(	N'Duplicate ResponseID(s) in Stage.Global_China_Sales_WithResponses.', 
						16,
						1
					 )
	...
	IF Exists(			
				...
				FROM		[Stage].[Global_China_Sales_WithResponses] gsc
				INNER JOIN	[China].[Sales_WithResponses] cr ON gsc.ResponseID = cr.ResponseID
			 )
				
			RAISERROR(	N'Non-unique ResponseID(s) in Stage.Global_China_Sales_WithResponses; Possible repeat of previous loaded cases.', 
						16,
						1
					 )	
END TRY
BEGIN CATCH
	...
	EXEC('
		SELECT *
		INTO [$(ErrorDB)].Stage.Global_China_Sales_WithResponses_' + @TimestampString + '
		FROM Stage.Global_China_Sales_WithResponses
	')
