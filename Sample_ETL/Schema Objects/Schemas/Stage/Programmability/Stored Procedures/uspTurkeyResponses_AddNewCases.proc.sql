CREATE PROCEDURE Stage.uspTurkeyResponses_AddNewCases
...
BEGIN TRY
	...
	BEGIN TRAN
		...
		WHERE ISNULL(TR.CaseID,0) = 0				-- V1.1, V1.2
		...
		-- WRITE BACK NEWLY CREATED CASE IDS TO TURKEYRESPONSES
		UPDATE TR
		SET TR.CaseID = AEBI.CaseID
		FROM Stage.TurkeyResponses TR
			INNER JOIN [$(SampleDB)].Event.AutomotiveEventBasedInterviews AEBI ON TR.EventID = AEBI.EventID
		...
		FROM @NewCases NC
			...
			INNER JOIN Stage.TurkeyResponses TR ON NC.CaseID = TR.CaseID
			LEFT JOIN [$(AuditDB)].Audit.Cases AC ON AC.AuditItemID = TR.AuditItemID
