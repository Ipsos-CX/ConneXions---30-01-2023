CREATE PROCEDURE [Stage].[uspStandardise_LandRover_Germany_Sales_VISTA]
AS
...
/*
	...
	1.0				$(ReleaseDate)		Simon Peacock		Created from [Prophet-ETL].dbo.uspStandardise_LandRover_Germany_Sales_VISTA
...
BEGIN TRY
	...
	UPDATE	Stage.LandRover_Germany_Sales_VISTA
	SET		ConvertedRegistrationDate = CONVERT( DATETIME, [Registration Date])		
	WHERE	ISDATE ( [Registration Date] ) = 1
	AND		LEN([Registration Date]) = 10

	-- Convert HandoverDate  in DateTime data type format
	UPDATE	Stage.LandRover_Germany_Sales_VISTA
	SET		ConvertedHandoverDate = CONVERT( DATETIME, [Handover Date])
	...
	UPDATE	Stage.LandRover_Germany_Sales_VISTA
	SET		ConvertedDateOfBirth = CONVERT( DATETIME, [Date of Birth]) 
	WHERE	ISDATE (  [Date of Birth] ) = 1
	AND		LEN([Date of Birth]) = 10
	AND		CombinedDateOfBirth IS NULL
	...
	--FROM	Stage.LandRover_Germany_Sales_VISTA
...
BEGIN CATCH
	...
	EXEC('
		SELECT *
		INTO [$(ErrorDB)].Stage.LandRover_Germany_Sales_VISTA' + @TimestampString + '
		FROM Stage.LandRover_Germany_Sales_VISTA
	')
