CREATE PROCEDURE Stage.uspStandardise_LandRover_Spain_Service 
AS
...
/*
	...
	1.1				2016-03-14		Chris Ledger		Change for preferred language
...
BEGIN TRY
	...
	UPDATE Stage.LandRover_Spain_Service
	...
	FROM		Stage.LandRover_Spain_Service SSV	
	...
	IF Exists( 
			...
			RAISERROR(	N'PreferredLanguageId cannot equal zero (Stage.LandRover_Spain_Service).', 
						16,
						1
					 )
...
BEGIN CATCH
	...
	EXEC('
		SELECT *
		INTO [$(ErrorDB)].Stage.LandRover_Spain_Service_' + @TimestampString + '
		FROM Stage.LandRover_Spain_Service
	')
