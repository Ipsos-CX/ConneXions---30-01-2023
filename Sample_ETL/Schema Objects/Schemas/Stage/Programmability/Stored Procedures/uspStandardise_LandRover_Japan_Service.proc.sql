...
CREATE PROCEDURE Stage.uspStandardise_LandRover_Japan_Service
AS
...
/*
	...
	1.0				$(ReleaseDate)		Simon Peacock		Created from [Prophet-ETL].dbo.uspSTANDARDISE_STAGE_LandRover_Japan_Service
...
BEGIN TRY
	...
	UPDATE Stage.LandRover_Japan_Service
    ...
    FROM    Stage.LandRover_Japan_Service
	WHERE   ISDATE ( SUBSTRING( ServiceEventDate, 1, 2) 
	               + '/' 
	               + SUBSTRING( ServiceEventDate, 3, 2) 
	               + '/' 
...
BEGIN CATCH
	...
	EXEC('
		SELECT *
		INTO [$(ErrorDB)].Stage.LandRover_Japan_Service_' + @TimestampString + '
		FROM Stage.LandRover_Japan_Service
	')
