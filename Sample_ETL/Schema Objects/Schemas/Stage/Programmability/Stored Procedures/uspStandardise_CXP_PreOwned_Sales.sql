CREATE PROCEDURE [dbo].[uspStandardise_CXP_PreOwned_Sales]
/*
	Purpose:		(I)		Convert the supplied dates from a text string to a valid DATETIME type
					(II)	Populate mandatory meta data
	
	Version			Date			Developer			Comment
	1.0				28/04/2021		Eddie Thomas		Created from Sample_ETL.uspStandardise_CXP_PreOwned_Sales
	1.1				20/05/2021		Eddie Thomas		Bugratacker 18207 - Blank Event_Type field values causes loader to bomb when copying data to VWT
...
BEGIN TRY
	...
	BEGIN 
			...
			FROM	Stage.CXP_PreOwned_Sales GS
	...
	UPDATE	Stage.CXP_PreOwned_Sales
	...
	BEGIN
			...
			FROM			Stage.CXP_PreOwned_Sales S 
	...
	IF Exists( 
			...
			RAISERROR(	N'Data in Stage.CXP_PreOwned_Sales has missing Meta-Data.', 
						16,
						1
					 )
END TRY
BEGIN CATCH
	...
	EXEC('
		SELECT *
		INTO [Sample_Errors].Stage.CXP_PreOwned_Sales_' + @TimestampString + '
		FROM Stage.CXP_PreOwned_Sales
	')
