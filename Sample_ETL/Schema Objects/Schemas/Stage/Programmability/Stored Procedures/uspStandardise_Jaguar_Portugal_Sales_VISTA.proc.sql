CREATE PROCEDURE [Stage].[uspStandardise_Jaguar_Portugal_Sales_VISTA]
AS
...
BEGIN TRY
	...
	UPDATE	Stage.Jaguar_Portugal_Sales_VISTA
	...
	SET			PreferredLanguageId =	CASE 
										...
										END					
	FROM		Stage.Jaguar_Portugal_Sales_VISTA PSV	
	...
	IF Exists( 
				SELECT	ID 
				FROM	Stage.Jaguar_Portugal_Sales_VISTA 
				WHERE	ISNULL(CountryID,0)	= 0	OR 
						ISNULL(PreferredLanguageId,0) =0
			)			 
			
			RAISERROR(	N'CountryID / PreferredLanguageId cannot equal zero (Stage.Jaguar_Portugal_Sales_VISTA).', 
						16,
						1
					 )
END TRY
BEGIN CATCH
	...
	EXEC('
		SELECT *
		INTO [$(ErrorDB)].Stage.Jaguar_Portugal_Sales_VISTA_' + @TimestampString + '
		FROM Stage.Jaguar_Portugal_Sales_VISTA
	')
