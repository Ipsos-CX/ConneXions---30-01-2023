...
CREATE PROCEDURE Stage.uspStandardise_LandRover_Germany_Service
AS
/*
	Purpose:	Convert the service date from a text string to a valid DATETIME type
	
	Version			Date			Developer			Comment
	1.0				$(ReleaseDate)		Simon Peacock		Created from [Prophet-ETL].dbo.uspSTANDARDISE_STAGE_LandRover_Germany_Service
...
BEGIN TRY
	...
	UPDATE Stage.LandRover_Germany_Service 
	...
	FROM	Stage.LandRover_Germany_Service
...
BEGIN CATCH
	...
	EXEC('
		SELECT *
		INTO [$(ErrorDB)].Stage.LandRover_Germany_Service_' + @TimestampString + '
		FROM Stage.LandRover_Germany_Service
	')
