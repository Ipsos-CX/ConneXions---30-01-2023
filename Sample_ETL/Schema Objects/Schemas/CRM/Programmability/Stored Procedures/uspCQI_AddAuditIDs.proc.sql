CREATE PROCEDURE CRM.uspCQI_AddAuditIDs
...
*/
	...
	BEGIN TRY
		...
		BEGIN TRAN
			...
			UPDATE CRM.CQI_ACCT_MKT_PERM
			SET AuditID = @AuditID
			WHERE AuditID IS NULL

			UPDATE CRM.CQI_ACCT_MKT_PERM_ITEM
			SET AuditID = @AuditID
			WHERE AuditID IS NULL

			UPDATE CRM.CQI_CNT_MKT_PERM
			SET AuditID = @AuditID
			WHERE AuditID IS NULL

			UPDATE CRM.CQI_CNT_MKT_PERM_ITEM
			SET AuditID = @AuditID
			WHERE AuditID IS NULL
