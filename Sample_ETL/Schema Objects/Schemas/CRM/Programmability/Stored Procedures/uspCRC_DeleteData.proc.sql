CREATE PROCEDURE CRM.uspCRC_DeleteData
...
*/
	...
	BEGIN TRY
		...
		BEGIN TRAN
			...
			DELETE
			FROM CRM.CRC_ACCT_MKT_PERM
			WHERE AuditID IS NULL
				OR AuditID = @AuditID

			DELETE
			FROM CRM.CRC_ACCT_MKT_PERM_ITEM
			WHERE AuditID IS NULL
				OR AuditID = @AuditID

			DELETE
			FROM CRM.CRC_CNT_MKT_PERM
			WHERE AuditID IS NULL
				OR AuditID = @AuditID

			DELETE
			FROM CRM.CRC_CNT_MKT_PERM_ITEM
			WHERE AuditID IS NULL
				OR AuditID = @AuditID
