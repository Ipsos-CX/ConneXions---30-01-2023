CREATE PROCEDURE   [CRM].[uspROADSIDE_SampleTrigSelections]
...
BEGIN TRY
	...
	SET SampleTriggeredSelectionReqID = (CASE	WHEN BMQ.CreateSelection = 1 AND BMQ.SampleTriggeredSelection = 1 THEN BMQ.QuestionnaireRequirementID
												ELSE 0 END)
	FROM CRM.RoadsideIncident_Roadside R
		INNER JOIN dbo.VWT V ON R.AuditItemID = V.AuditItemID
	...
	----------------------------------------------------------------------------------------------------
	UPDATE R 
	SET  R.SampleTriggeredSelectionReqID = V.SampleTriggeredSelectionReqID
	FROM CRM.RoadsideIncident_Roadside R
		INNER JOIN dbo.VWT V ON R.AuditItemID = V.AuditItemID
