CREATE PROCEDURE CRM.uspROADSIDE_DeleteData
...
*/
	...
	BEGIN TRY
		...
		BEGIN TRAN
			...
			DELETE
			FROM CRM.ROADSIDE_ACCT_MKT_PERM
			WHERE AuditID IS NULL
				OR AuditID = @AuditID

			DELETE
			FROM CRM.ROADSIDE_ACCT_MKT_PERM_ITEM
			WHERE AuditID IS NULL
				OR AuditID = @AuditID

			DELETE
			FROM CRM.ROADSIDE_CNT_MKT_PERM
			WHERE AuditID IS NULL
				OR AuditID = @AuditID

			DELETE
			FROM CRM.ROADSIDE_CNT_MKT_PERM_ITEM
			WHERE AuditID IS NULL
				OR AuditID = @AuditID
