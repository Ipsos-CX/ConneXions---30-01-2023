CREATE PROCEDURE   [CRM].[uspROADSIDE_DataConversions]
...
LIVE	1.1			Eddie Thomas		13/09/2017		BUG 14235:	CRM R4.0 - Setup Luxembourg. Luxembourg records are updated to be Belgium. 
																	This is what happens in the combined Roadside loader
LIVE	1.2			Chris Ross			15/11/2018		BUG 15112: Include convert of ROADSIDE_INCIDENT_DATE 			
...
LIVE	1.4			Chris Ledger		24/08/2021		TASK 502: Change to RoadsideIncident_Roadside and add date conversion of DATEOFCONSENT and loading of permission fields
LIVE	1.5			Chris Ledger		27/01/2022		TASK 502: Only allow YA2 for Italy and use @AuditID to filter updating of permission fields
*/
...
BEGIN TRY
	...
	---------------------------------------------------------------------------------------------------
	-- V1.5 GET COUNTRYCODE FROM AUDITID
	---------------------------------------------------------------------------------------------------
	...
	SET Converted_VISTACONTRACT_HANDOVER_DATE = NULLIF(VISTACONTRACT_HANDOVER_DATE, '0000-00-00'),
		...
		Converted_ROADSIDE_INCIDENT_DATE = NULLIF(ROADSIDE_INCIDENT_DATE, '0000-00-00')						-- V1.2
	FROM CRM.RoadsideIncident_Roadside RI
	WHERE RI.DateTransferredToVWT IS NULL																	-- V1.3
	
	
	-- V1.1
	UPDATE RI
    SET RI.ACCT_COUNTRY_CODE = CASE RI.ACCT_COUNTRY_CODE	WHEN 'LU' THEN 'BE'
															ELSE RI.ACCT_COUNTRY_CODE END,
		RI.ROADSIDE_INCIDENT_COUNTRY = CASE RI.ROADSIDE_INCIDENT_COUNTRY	WHEN 'LU' THEN 'BE'
																			ELSE RI.ROADSIDE_INCIDENT_COUNTRY END
    FROM CRM.RoadsideIncident_Roadside RI
	WHERE RI.DateTransferredToVWT IS NULL		-- V1.3
		

	---------------------------------------------------------------------------------------------------
	--- Update ROADSIDE_ACCT_MKT_PERM_ITEM.DATEOFCONSENT										-- V1.4
	---------------------------------------------------------------------------------------------------
	UPDATE AMPI
	...
	FROM CRM.RoadsideIncident_Roadside RI	
		...
		INNER JOIN CRM.ROADSIDE_ACCT_MKT_PERM_ITEM AMPI ON RI.AuditID = AMPI.AuditID
														AND AMP.ACCT_MKT_PERM_Id = AMPI.ACCT_MKT_PERM_Id
	WHERE RI.AuditID = @AuditID																	-- V1.5
	...
	FROM CRM.RoadsideIncident_Roadside RI	
		...
		INNER JOIN CRM.ROADSIDE_CNT_MKT_PERM_ITEM CMPI ON RI.AuditID = CMPI.AuditID
															AND CMP.CNT_MKT_PERM_Id = CMPI.CNT_MKT_PERM_Id
	WHERE RI.AuditID = @AuditID																	-- V1.5


	---------------------------------------------------------------------------------------------------
	--- Update Personal Permission Fields														-- V1.4
	---------------------------------------------------------------------------------------------------
	;WITH CTE_PIVOT AS
	(
		SELECT ID,
		...
		(
			...
			FROM CRM.RoadsideIncident_Roadside RI
				INNER JOIN CRM.ROADSIDE_ACCT_MKT_PERM AMP ON RI.AuditID = AMP.AuditID
																	AND RI.item_Id = AMP.item_Id
				LEFT JOIN CRM.ROADSIDE_ACCT_MKT_PERM_ITEM AMPI ON RI.AuditID = AMPI.AuditID
																	AND AMP.ACCT_MKT_PERM_Id = AMPI.ACCT_MKT_PERM_Id
			WHERE RI.AuditID = @AuditID																									-- V1.5
				AND ((@CountryCode = 'IT' AND AMPI.COMMCHANNEL = 'YA2')																	-- V1.5
					OR (@CountryCode <> 'IT' AND AMPI.COMMCHANNEL IN ('Y06','Y16','Y26','Y36','Y46','Y56','Y66','Y76','Y86','Y96')))	-- V1.5
		) D
		PIVOT
		(
			MAX(CONSENT)
	...
	UPDATE RI
	SET RI.JAGPHONESURVEYSRESEARCH = CASE	WHEN P.CUSTOMERSATISFACTIONSURVEY IS NOT NULL THEN P.CUSTOMERSATISFACTIONSURVEY
											ELSE P.JAGPHONESURVEYSRESEARCH END,
		RI.LRPHONESURVEYSRESEARCH = CASE	WHEN P.CUSTOMERSATISFACTIONSURVEY IS NOT NULL THEN P.CUSTOMERSATISFACTIONSURVEY
											ELSE P.LRPHONESURVEYSRESEARCH END,
		RI.JAGEMAILSURVEYSRESEARCH = CASE	WHEN P.CUSTOMERSATISFACTIONSURVEY IS NOT NULL THEN P.CUSTOMERSATISFACTIONSURVEY
											ELSE P.JAGEMAILSURVEYSRESEARCH END,
		RI.LREMAILSURVEYSRESEARCH = CASE	WHEN P.CUSTOMERSATISFACTIONSURVEY IS NOT NULL THEN P.CUSTOMERSATISFACTIONSURVEY
											ELSE P.LREMAILSURVEYSRESEARCH END,
		RI.JAGPOSTSURVEYSRESEARCH = CASE	WHEN P.CUSTOMERSATISFACTIONSURVEY IS NOT NULL THEN P.CUSTOMERSATISFACTIONSURVEY
											ELSE P.JAGPOSTSURVEYSRESEARCH END,
		RI.LRPOSTSURVEYSRESEARCH = CASE	WHEN P.CUSTOMERSATISFACTIONSURVEY IS NOT NULL THEN P.CUSTOMERSATISFACTIONSURVEY
										ELSE P.LRPOSTSURVEYSRESEARCH END,
		RI.JAGSMSSURVEYSRESEARCH = CASE	WHEN P.CUSTOMERSATISFACTIONSURVEY IS NOT NULL THEN P.CUSTOMERSATISFACTIONSURVEY
										ELSE P.JAGSMSSURVEYSRESEARCH END,
		RI.LRSMSSURVEYSRESEARCH = CASE	WHEN P.CUSTOMERSATISFACTIONSURVEY IS NOT NULL THEN P.CUSTOMERSATISFACTIONSURVEY
										ELSE P.LRSMSSURVEYSRESEARCH END,
		RI.JAGDIGITALSURVEYSRESEARCH = CASE	WHEN P.CUSTOMERSATISFACTIONSURVEY IS NOT NULL THEN P.CUSTOMERSATISFACTIONSURVEY
											ELSE P.JAGDIGITALSURVEYSRESEARCH END,
		RI.LRDIGITALSURVEYSRESEARCH = CASE	WHEN P.CUSTOMERSATISFACTIONSURVEY IS NOT NULL THEN P.CUSTOMERSATISFACTIONSURVEY
											ELSE P.LRDIGITALSURVEYSRESEARCH END
	FROM CRM.RoadsideIncident_Roadside RI
		INNER JOIN CTE_PIVOT P ON RI.AuditID = P.AuditID
									AND RI.AuditItemID = P.AuditItemID
									AND RI.item_Id = P.item_Id 
	WHERE RI.AuditID = @AuditID																	-- V1.5


	---------------------------------------------------------------------------------------------------
	--- Update Company Permission Fields														-- V1.4
	---------------------------------------------------------------------------------------------------
	;WITH CTE_PIVOT AS
	(
		SELECT ID,
		...
		(
			...
			INNER JOIN CRM.ROADSIDE_CNT_MKT_PERM CMP ON RI.AuditID = CMP.AuditID
																AND RI.item_Id = CMP.item_Id
			LEFT JOIN CRM.ROADSIDE_CNT_MKT_PERM_ITEM CMPI ON RI.AuditID = CMPI.AuditID
																AND CMP.CNT_MKT_PERM_Id = CMPI.CNT_MKT_PERM_Id
			WHERE RI.AuditID = @AuditID																									-- V1.5
				AND ((@CountryCode = 'IT' AND CMPI.COMMCHANNEL = 'YA2')																	-- V1.5
					OR (@CountryCode <> 'IT' AND CMPI.COMMCHANNEL IN ('Y06','Y16','Y26','Y36','Y46','Y56','Y66','Y76','Y86','Y96')))	-- V1.5
		) D
		PIVOT
		(
		  MAX(CONSENT)
	...
	UPDATE RI
	SET RI.CNT_JAGPHONESURVEYSRESEARCH = CASE	WHEN P.CNT_CUSTOMERSATISFACTIONSURVEY IS NOT NULL THEN P.CNT_CUSTOMERSATISFACTIONSURVEY
												ELSE P.CNT_JAGPHONESURVEYSRESEARCH END,
		RI.CNT_LRPHONESURVEYSRESEARCH = CASE	WHEN P.CNT_CUSTOMERSATISFACTIONSURVEY IS NOT NULL THEN P.CNT_CUSTOMERSATISFACTIONSURVEY
												ELSE P.CNT_LRPHONESURVEYSRESEARCH END,
		RI.CNT_JAGEMAILSURVEYSRESEARCH = CASE	WHEN P.CNT_CUSTOMERSATISFACTIONSURVEY IS NOT NULL THEN P.CNT_CUSTOMERSATISFACTIONSURVEY
												ELSE P.CNT_JAGEMAILSURVEYSRESEARCH END,
		RI.CNT_LREMAILSURVEYSRESEARCH = CASE	WHEN P.CNT_CUSTOMERSATISFACTIONSURVEY IS NOT NULL THEN P.CNT_CUSTOMERSATISFACTIONSURVEY
												ELSE P.CNT_LREMAILSURVEYSRESEARCH END,
		RI.CNT_JAGPOSTSURVEYSRESEARCH = CASE	WHEN P.CNT_CUSTOMERSATISFACTIONSURVEY IS NOT NULL THEN P.CNT_CUSTOMERSATISFACTIONSURVEY
												ELSE P.CNT_JAGPOSTSURVEYSRESEARCH END,
		RI.CNT_LRPOSTSURVEYSRESEARCH = CASE	WHEN P.CNT_CUSTOMERSATISFACTIONSURVEY IS NOT NULL THEN P.CNT_CUSTOMERSATISFACTIONSURVEY
											ELSE P.CNT_LRPOSTSURVEYSRESEARCH END,
		RI.CNT_JAGSMSSURVEYSRESEARCH = CASE	WHEN P.CNT_CUSTOMERSATISFACTIONSURVEY IS NOT NULL THEN P.CNT_CUSTOMERSATISFACTIONSURVEY
											ELSE P.CNT_JAGSMSSURVEYSRESEARCH END,
		RI.CNT_LRSMSSURVEYSRESEARCH = CASE	WHEN P.CNT_CUSTOMERSATISFACTIONSURVEY IS NOT NULL THEN P.CNT_CUSTOMERSATISFACTIONSURVEY
											ELSE P.CNT_LRSMSSURVEYSRESEARCH END,
		RI.CNT_JAGDIGITALSURVEYSRESEARCH = CASE	WHEN P.CNT_CUSTOMERSATISFACTIONSURVEY IS NOT NULL THEN P.CNT_CUSTOMERSATISFACTIONSURVEY
												ELSE P.CNT_JAGDIGITALSURVEYSRESEARCH END,
		RI.CNT_LRDIGITALSURVEYSRESEARCH = CASE	WHEN P.CNT_CUSTOMERSATISFACTIONSURVEY IS NOT NULL THEN P.CNT_CUSTOMERSATISFACTIONSURVEY
	...
	---------------------------------------------------------------------------------------------------
	--- Calculate Salutation (where applicable)													-- V1.3
	---------------------------------------------------------------------------------------------------
	...
	SELECT R.AuditItemID,
		CASE	WHEN R.ACCT_ACCT_TYPE = 'Person' THEN RTRIM(ISNULL(R.ACCT_TITLE, ''))				
				ELSE RTRIM(ISNULL(R.CNT_TITLE, '')) END AS TITLE,
		CASE	WHEN R.ACCT_ACCT_TYPE = 'Person' THEN RTRIM(ISNULL(R.ACCT_NAME_PREFIX, ''))			
				ELSE RTRIM(ISNULL(R.CNT_NAME_PREFIX, '')) END AS NAME_PREFIX,
		CASE	WHEN R.ACCT_ACCT_TYPE = 'Person' THEN RTRIM(ISNULL(R.ACCT_NON_ACADEMIC_TITLE, ''))	
				ELSE RTRIM(ISNULL(R.CNT_NON_ACADEMIC_TITLE, '')) END AS NON_ACADEMIC_TITLE,
		CASE	WHEN R.ACCT_ACCT_TYPE = 'Person' THEN RTRIM(ISNULL(R.ACCT_LAST_NAME, ''))			
				ELSE RTRIM(ISNULL(R.CNT_LAST_NAME, '')) END AS LAST_NAME,
		CASE	WHEN R.ACCT_ACCT_TYPE = 'Person' THEN RTRIM(ISNULL(R.ACCT_PREF_LAST_NAME, ''))		
				ELSE RTRIM(ISNULL(R.CNT_PREF_LAST_NAME, '')) END AS PREF_LAST_NAME,
		ACCT_COUNTRY_CODE
	FROM CRM.RoadsideIncident_Roadside R
		INNER JOIN CRM.SalutationCountryDefaults SCD ON SCD.CountryISOAlpha2 = R.ACCT_COUNTRY_CODE 
														AND SCD.Enabled = 1
	WHERE R.DateTransferredToVWT IS NULL	
		AND R.ACCT_COUNTRY_CODE IS NOT NULL
	...
	UPDATE R
	SET R.Calculated_Salutation = CASE WHEN SC.SetToDefault = 1 OR DN.SalutationDearNameID IS NULL 
										...
										ELSE LTRIM(REPLACE(
													REPLACE(DN.DearName + ' ' + 
															...
															SAL_LAST_NAME
															, '   ', ' '), '  ', ' ') )	END,
		R.Calculated_Title = LTRIM(REPLACE(
										REPLACE(ISNULL(TRN_TITLE, '') + ' ' +
												ISNULL(TRN_NAME_PREFIX, '') + ' ' +
												ISNULL(TRN_NON_ACADEMIC_TITLE, '')
												, '   ', ' '), '  ', ' ') )
	FROM #SalutationCalculation SC
		INNER JOIN CRM.RoadsideIncident_Roadside R ON R.AuditItemID = SC.AuditItemID
		INNER JOIN CRM.SalutationCountryDefaults SCD ON SCD.CountryISOAlpha2 = R.ACCT_COUNTRY_CODE  
		LEFT JOIN CRM.SalutationDearNames DN ON DN.CountryISOAlpha2 = R.ACCT_COUNTRY_CODE 
