CREATE PROCEDURE [CRM].[uspROADSIDE_AddAuditItemIDs]
...
*/
	...
	BEGIN TRY
		...
		BEGIN TRAN
			...
			UPDATE RI
				SET RI.AuditItemID = AI.AuditItemID
			FROM CRM.RoadsideIncident_Roadside RI
				INNER JOIN cteAuditItems AI ON RI.AuditID = AI.AuditID 
												AND RI.PhysicalRowID = AI.PhysicalRowID;
			...
			WHERE NOT EXISTS (	SELECT 1 
								...
								WHERE AI.AuditItemID = RI.AuditItemID)
			...
			UPDATE AMP
			SET AMP.AuditItemID = RI.AuditItemID
			FROM CRM.RoadsideIncident_Roadside RI
				INNER JOIN CRM.ROADSIDE_ACCT_MKT_PERM AMP ON RI.AuditID = AMP.AuditID
														AND RI.item_Id = AMP.item_Id
			...
			FROM CRM.RoadsideIncident_Roadside RI
				INNER JOIN CRM.ROADSIDE_ACCT_MKT_PERM AMP ON RI.AuditID = AMP.AuditID
															AND RI.item_Id = AMP.item_Id
				INNER JOIN CRM.ROADSIDE_ACCT_MKT_PERM_ITEM AMPI ON AMP.AuditID = AMPI.AuditID
																AND AMP.ACCT_MKT_PERM_Id = AMPI.ACCT_MKT_PERM_Id
			WHERE AMPI.AuditItemID IS NULL

			UPDATE CMP
			SET CMP.AuditItemID = RI.AuditItemID
			FROM CRM.RoadsideIncident_Roadside RI
				INNER JOIN CRM.ROADSIDE_CNT_MKT_PERM CMP ON RI.AuditID = CMP.AuditID
														AND RI.item_Id = CMP.item_Id
			...
			FROM CRM.RoadsideIncident_Roadside RI
				INNER JOIN CRM.ROADSIDE_CNT_MKT_PERM CMP ON RI.AuditID = CMP.AuditID
															AND RI.item_Id = CMP.item_Id
				INNER JOIN CRM.ROADSIDE_CNT_MKT_PERM_ITEM CMPI ON CMP.AuditID = CMPI.AuditID
