CREATE VIEW [CRM].[vwCRCCall_TransferToVWT]
AS 
...
/*
	...
	1.19		Ben King			15/02/2021		BUG 18106: Belgium Preferred Language
	1.20		Chris Ledger		16/06/2021		TASK 411: Add Andora to Spain
	...
	WITH CTE_Manfacturers AS		-- Ensures only single manufacturer returned for each row
	 (
		...
		FROM CRM.CRCCall_Call CRM
			LEFT JOIN [$(SampleDB)].Vehicle.VehicleMatchingStrings VMS ON CRM.CASE_VEH_VIN_NUMBER LIKE VMS.VehicleMatchingString	
...
--		ACCT_PREFIX_2,
		...
		VEH_NUM_OF_OWNERS_RELATIONSHIP,				-- V1.1 prev VEH_NUMBER_OF_OWNERS_RELATIONSHIPS		, VEH_VIN
		CASE_VEH_REG_PLATE,
		CASE_VEH_VIN_NUMBER,
		CASE_CASE_CREATION_DATE,
		CASE_CASE_SOLVED_DATE,
		CASE_SECON_DEALER_CODE_OF_DEAL,			-- V1.1 prev CASE_SECON_DEALER_CODE_OF_DEALER
		CASE_BRAND,
		CASE_BRAND_CODE,
		...
		CASE_CASE_EMPL_RESPONSIBLE_NAM,																-- V1.1
		CASE_CASE_DESC,																				-- V1.1
		CASE_CASE_ID,																				-- V1.1
		...
		CASE	WHEN LEFT(REPLACE(CASE_VEH_VIN_NUMBER, ' ', ''),3) = 'SAJ' THEN 2							-- V1.6
				WHEN LEFT(REPLACE(CASE_VEH_VIN_NUMBER, ' ', ''),3) = 'SAL' THEN 3							-- V1.6
				WHEN LEN(REPLACE(CASE_VEH_VIN_NUMBER, ' ', '')) = 17 THEN ISNULL(M.ManufacturerPartyID,0)	-- V1.6
		...
		ACCT_CORRESPONDENCE_LANGUAGE																-- V1.19
	FROM CRM.CRCCall_Call CRM
