CREATE VIEW [CRM].[vwCQI_TransferToVWT]
AS 

/*
	Purpose:	Identify which CRM sourced CQI Sales events have yet to be transferred to VWT
	...
	1.21		Chris Ledger		05/05/2021		TASK 411: Copied from vwVISTA_TransferToVWT
	...
	1.24		Chris Ledger		07/09/2021		TASK 595: Pick up ACCT_COUNTRY_CODE & ISOAlpha2LanguageCode from Sample File
*/
	...
	(
		...
		FROM CRM.CQI CRM
			LEFT JOIN [$(SampleDB)].Vehicle.VehicleMatchingStrings VMS ON CRM.VEH_VIN LIKE VMS.VehicleMatchingString	
	...
	(
		SELECT DISTINCT CRM.AuditItemID, 
			...
			COALESCE(CRM.ACCT_PREF_LANGUAGE_CODE, L.ISOAlpha2) AS ISOAlpha2LanguageCode
		FROM CRM.CQI CRM
...
--		ACCT_PREFIX_2,
		...
		C.ISOAlpha2LanguageCode,																	-- V1.16, V1.24
	...
	WHERE CRM.DateTransferredToVWT IS NULL															-- V1.6
