CREATE PROCEDURE [China].[uspMatchPartiesUsingEmails]
...
BEGIN TRY
		...
		SELECT	CONVERT(BIGINT, re.ID) AS RoadsideID, 
				re.EmailAddress1 AS EmailAddress, 
				re.Firstname, 
				re.SurnameField1, 
				re.SurnameField2
		FROM [China].[Roadside_WithResponses] re
		WHERE CountryID IN (SELECT CountryID FROM [$(SampleDB)].dbo.Markets WHERE ISNULL(AltRoadsideEmailMatching, 0) = 1)  -- Only markets that have alternative Email matched flagged
			...
			--------------------------------------------------------------------------------------------------------------------------------------------------------------------
		UNION
		SELECT CONVERT(BIGINT, re.ID) AS RoadsideID, re.EmailAddress2 AS EmailAddress, re.Firstname, re.SurnameField1, re.SurnameField2
		FROM [China].[Roadside_WithResponses] re
		...
		INNER JOIN  [China].[Roadside_WithResponses] re ON re.ID = em.RoadsideID 
