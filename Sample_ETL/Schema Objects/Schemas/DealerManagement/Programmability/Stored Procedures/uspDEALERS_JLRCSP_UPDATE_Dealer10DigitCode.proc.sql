CREATE PROCEDURE [DealerManagement].[uspDEALERS_JLRCSP_UPDATE_Dealer10DigitCode]

AS
SET NOCOUNT ON
	SET QUOTED_IDENTIFIER ON

	-- Purpose: Update Dealer10DigitCode name in both the sample model and the flattened dealer hierarchy table
	--
	--
	-- Version		Developer			Date			Comment
	-- 1.0			Chris Ledger		17/01/2020		BUG 16793 - DDB - 10 Digit Code to be added
	...
	-- DECLARE LOCAL VARIABLES 
		...
		-- POPULATE WORKSPACE TABLE TO HOLD AUDIT TRAIL INFO
			...
			CREATE TABLE #AuditTrail
				(
					ID INT
					, IP_IP_Dealer10DigitCodeChangeID INT
					, ID_Hierarchy INT
					, PartyID INT
					, RoleTypeID INT
					, PartyRoleID INT
			...
			INSERT INTO #AuditTrail
				...
				(
					ID
					, IP_IP_Dealer10DigitCodeChangeID
					, ID_Hierarchy
					, PartyID
					, RoleTypeID
					, PartyRoleID
					...
					SELECT 
						ROW_NUMBER() OVER (ORDER BY UON.IP_Dealer10DigitCodeChangeID) ID
						, UON.IP_Dealer10DigitCodeChangeID
						, UON.ID
						, DU.PartyID
						, DU.RoleTypeID
						, DU.PartyRoleID
						, NULL AS AuditID
						, NULL AS AuditItemID
					FROM DealerManagement.DEALERS_JLRCSP_UPDATE_Dealer10DigitCode UON
		...
		INSERT INTO [$(AuditDB)].OWAP.vwDA_Sessions
			...
			SELECT 
				0 AS AuditID
				, DU.PartyRoleID
				, U.UserName + ' - Dealer10DigitCode update - ' + CAST(DU.GUID AS NVARCHAR(100)) AS SessionID
				, GETDATE()
		...
		-- WRITE BACK THE CREATED AUDITID USING THE UNIQUE GUID/SESSIONID
			...
			INNER JOIN [$(AuditDB)].OWAP.Sessions S ON U.UserName + ' - Dealer10DigitCode update - ' + CAST(DU.GUID AS VARCHAR(100))= S.SessionID
		...
		-- WRITE AUDITITEMIDS BACK TO DEALER PAG TABLE
			...
			FROM #AuditTrail AT
			INNER JOIN DealerManagement.DEALERS_JLRCSP_UPDATE_Dealer10DigitCode UON ON AT.IP_IP_Dealer10DigitCodeChangeID = UON.IP_Dealer10DigitCodeChangeID

	--PERFORM UPDATE ON DIMENSION TABLE ROWS
	
		UPDATE D 
			SET D.Dealer10DigitCode = UON.[Dealer10DigitCode]
		FROM DealerManagement.DEALERS_JLRCSP_UPDATE_Dealer10DigitCode UON
		INNER JOIN [$(SampleDB)].dbo.DW_JLRCSPDealers D ON UON.ID = D.ID
		WHERE UON.IP_ProcessedDate IS NULL
		AND UON.IP_DataValidated = 1
		AND (UON.Dealer10DigitCode <> ISNULL(D.Dealer10DigitCode,''))
	...
	-- STAMP THE RECORDS AS PROCESSES
		...
		FROM #AuditTrail D
		INNER JOIN DealerManagement.DEALERS_JLRCSP_UPDATE_Dealer10DigitCode UON ON D.AuditItemID = UON.IP_AuditItemID
