CREATE PROCEDURE [DealerManagement].[uspDEALERS_JLRCSP_UPDATE_OutletSubNationalRegion]

AS
SET NOCOUNT ON
SET QUOTED_IDENTIFIER ON

	-- Purpose: Update outlet sub-national region details in the flattened dealer hierarchy table
	--
	--
	...
	-- 1.2			Chris Ross			17/12/2012		11082 - Modified to ignore the Dealer ID matching and go with the 
	--													more generic Outlet Code so that multiple (inactive) parties will get 
	--													processed, rather than just one.  (Checking for multiple 'live' parties 
	--													already part of verification trigger.
	...
	-- DECLARE LOCAL VARIABLES 
			...
			INSERT INTO #AuditTrail
				...
				(
					...
					SELECT 
						...
						, NULL AS AuditID
						, NULL AS AuditItemID
					FROM DealerManagement.DEALERS_JLRCSP_UPDATE_SubNationalRegion USNR
		...
		-- WRITE AUDITITEMIDS BACK TO DEALER APPOINTMENT TABLE
			...
			FROM #AuditTrail AT
			INNER JOIN DealerManagement.DEALERS_JLRCSP_UPDATE_SubNationalRegion USNR ON AT.IP_SubNationalRegionChangeID = USNR.IP_SubNationalRegionChangeID
	...
	-- STAMP THE RECORDS AS PROCESSES
		...
		FROM #AuditTrail AT
		INNER JOIN DealerManagement.DEALERS_JLRCSP_UPDATE_SubNationalRegion USNR ON AT.AuditItemID = USNR.IP_AuditItemID
