CREATE  PROCEDURE [dbo].[uspVWT_ExtractStreetNumber]
...
/*
	Purpose:	Extract Street Number from Street field.
				...
				DECIDED TO DO THIS FOR UK, Canada and Australia ONLY AS KNOWLEDGE OF ROW COUNTRIES ADDRESSES NOT COMPREHENSIVE ENOUGH.
				
				FROM					DATA								->		TO			
				StreetAndNumberOrig		Street Number						->		VWT.StreetNumber
				StreetAndNumberOrig		Street (incl. Prefix/Suffix)		->		VWT.StreetOrig	
	...
	1.0				$(ReleaseDate)		Simon Peacock		Created from [Prophet-ETL].dbo.uspSTANDARDISEADDRESS_ExtractStreetNumber
...
BEGIN TRY
	...
	SELECT AuditItemID, CountryID, StreetAndNumberOrig
	FROM VWT
	...
	UPDATE VWT
	SET VWT.StreetOrig = O.OutputString
	FROM  VWT
