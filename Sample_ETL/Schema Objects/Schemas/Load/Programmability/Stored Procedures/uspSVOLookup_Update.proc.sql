CREATE PROCEDURE [Load].[uspSVOLookup_Update]
	
AS

/*
	Purpose: Update SVO Lookup table
...
BEGIN TRY
	...
	1.	For the Sale Type issue, if two rows in the file for same Vin – 1 row with ‘SV’ and 1 row with ‘SV Bespoke’ then the row with ‘SV Bespoke’ 
		is used and the row with ‘SV’ is discarded/ignored. 
	...
	*/
	...
	FROM	(	SELECT Vin,  
				...
				FROM (	SELECT Vin,
							...
							ROW_NUMBER() OVER (	PARTITION BY Vin																					-- V1.1
												ORDER BY CASE	WHEN SaleType NOT IN ('SV','SV Bespoke','Bespoke','Core Bespoke') THEN 1			-- V1.1 V1.2
				...
				GROUP BY Vin) T2 INNER JOIN (	SELECT Vin,
													SaleType,
													ROW_NUMBER() OVER (	PARTITION BY Vin 															-- V1.1
																		ORDER BY CASE	WHEN SaleType NOT IN ('SV','SV Bespoke','Bespoke') THEN 1	-- V1.1 V1.2
																						WHEN SaleType ='Bespoke' THEN 2
																						WHEN SaleType ='SV' THEN 3 
																						WHEN SaleType ='SV Bespoke' THEN 4							-- V1.1 V1.2
																						WHEN SaleType ='Core Bespoke' THEN 5	END ASC) AS RowNum	-- V1.3
	...
	FROM Stage.SVOLookup LK
		INNER JOIN (	SELECT MIN(AuditItemID) AS AuditItemID, 
							LK.Vin
						FROM Stage.SVOLookup LK INNER JOIN (	SELECT DISTINCT Vin,
																	FeatureId
	...
	BEGIN TRAN
		...
		-- UPDATE THE LOOKUP		
		UPDATE LK
		SET	LK.ShortVin = ST.ShortVin,
			LK.ModelYearDescription = ST.ModelYearDescription,
			LK.ModelDescription = ST.ModelDescription,
			LK.ExteriorColourDescription = ST.ExteriorColourDescription,
			LK.TrimDescription	= ST.TrimDescription,
			LK.TransmissionDescription	= ST.TransmissionDescription,
			LK.TrimPackDescription	= ST.TrimPackDescription,
			LK.VistaMarketDesc = ST.VistaMarketDesc,
			LK.DealerCICode = ST.DealerCICode,
			LK.PartnerUniqueId = ST.PartnerUniqueId,
			LK.PartnerUniqueIdDesc = ST.PartnerUniqueIdDesc,
			LK.CommonTypeOfSaleDesc = ST.CommonTypeOfSaleDesc,
			LK.CommonOrderNo = ST.CommonOrderNo,
			LK.NscOrderNumber = ST.NscOrderNumber,
			LK.FactoryOrderNumber = ST.FactoryOrderNumber,
			LK.CommonStatusPointDesc = ST.CommonStatusPointDesc,
			LK.BuildDate = ST.BuildDate,
			LK.DeliveredDate = ST.DeliveredDate,
			LK.CustomerHandoverDate = ST.CustomerHandoverDate,
			LK.FeatureId = ST.FeatureId,
			LK.FeatureDesc = ST.FeatureDesc,
			LK.SaleType = ST.SaleType,
			LK.SVOTypeID = SVT.SVOTypeID,
			LK.Paint = ST.Paint
		FROM Lookup.SVOLookup LK
			INNER JOIN #Final_Deduped ST ON LK.Vin = ST.Vin
			LEFT JOIN dbo.SVOTypes SVT ON LTRIM(RTRIM(ST.SaleType)) = SVT.SVODescription
		WHERE LEN(ISNULL(ST.Vin,'')) > 0
