CREATE PROCEDURE [Load].[uspJLRManUserRapidBlackListedEmails]

AS

/*
	Purpose:	Load Black Listed emails from IPSOS_JLRManagementUsers & IPSOS_JLRRetailerUsers files
	
	Release		Version			Date			Developer			Comment
	LIVE			1.0			2022-12-05		Ben King    		TASK 866 - 19490 - Add JLR Employees to the Excluded Email list
...
BEGIN TRY
			...
			SELECT DISTINCT
					...
					GETDATE() AS FromDate
			FROM	[Stage].[JLRManagementUsersRapidMiner]	F
